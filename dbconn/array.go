...
// Multidimensional array values are rejected with an error, as pq did;
// note that pgtype's []string scanning silently flattens them instead,
// which is why this does not delegate to pgtype.
...
func (a *StringArray) scanBytes(src []byte) error {
	...
	for i < len(body) {
		if body[i] == '{' {
			// A brace in element position means a nested array. The server
			// only emits this for multidimensional values, which cannot be
			// represented in a flat []string; parsing on would silently
			// shred them into fragments like "{a" and "b}".
			return fmt.Errorf("dbconn: cannot scan multidimensional array %q into StringArray", src)
		}
		if body[i] == '"' {
			i++
			buf := make([]byte, 0, 16)
